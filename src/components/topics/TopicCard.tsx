...
export function TopicCard({ topic, done, onOpen }: TopicCardProps) {
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="group relative flex h-full w-full flex-col overflow-hidden rounded-2xl border border-gray/80 bg-white text-right shadow-ambient transition-all duration-200 hover:-translate-y-0.5 hover:shadow-ambient-lg cursor-pointer active:scale-[0.98]"
    >
      {done && (
        <span
          aria-hidden
